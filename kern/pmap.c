...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, (uintptr_t) KERNBASE, (0xffffffff - KERNBASE + 0x1), (physaddr_t) 0,  PTE_W);
...
{
	struct PageInfo * alloc_page_info = page_free_list;
	if (page_free_list == NULL) {
		return NULL;
	}
	if (alloc_flags & ALLOC_ZERO) {
		memset(page2kva(alloc_page_info), 0, PGSIZE);
	}

	page_free_list = page_free_list->pp_link;
	alloc_page_info->pp_link = NULL;
	return alloc_page_info;
}
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0 || pp->pp_link != NULL) {
		panic("pp_ref should be zero and pp_link should be NULL");
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(*pde & PTE_P)) {
		if (create == false) {
			...
			pginfo_alloc = page_alloc(ALLOC_ZERO);
			if (pginfo_alloc == NULL) {
				return NULL;
			}
			pginfo_alloc->pp_ref = 1;
			phy_addr = page2pa(pginfo_alloc);
			*pde = phy_addr | PTE_P | PTE_W;
		}
	} 
	pgtb = KADDR(PTE_ADDR(*pde));
	return pgtb + PTX(va);
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (i = 0; i < size / PGSIZE; i++) {
		...
		*pte = (pa + i * PGSIZE) | (perm | PTE_P);
		pde = pgdir + PDX((void *)(va + i * PGSIZE)); 
		*pde = (*pde) | perm | PTE_P; 
	}
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	if ((unsigned int)(va + len) >= ULIM || (va + len) < va) {
		user_mem_check_addr = (unsigned int) va >= ULIM ? (uintptr_t) va : ULIM;
		return -E_FAULT;
	}

	for (start; start < end; start += PGSIZE) {
		pte = pgdir_walk(env->env_pgdir, start, false);
